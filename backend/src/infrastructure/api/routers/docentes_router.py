...
def crear_docente(
    ...
    unidad_id = _unidad_filtro(current_user)
    logger.info("Intentando crear docente", context={"unidad_id": unidad_id, "nombre": docente.nombre}, trace_id=trace_id)
    try:
        docente_creado = docentes_service.crear_docente(db, docente, unidad_id=unidad_id)
        logger.info("Docente creado", context={"docente_id": docente_creado.id, "unidad_id": unidad_id}, trace_id=trace_id)
        return docente_creado
    except Exception as e:
        logger.warning("Error al crear docente", context={"error": str(e)}, trace_id=trace_id)
        raise HTTPException(status_code=400, detail=str(e))
